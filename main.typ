...
#figure(
  ...
  #ref(<1ac0d7acb204441885a656784e118d78>)が定める圏を集合の圏$"Set"$とする。
] <7c636bd681584a50a836d01876f298c2>

== 群の圏

$"Set"$は全ての集合を対象とするが、これに適当な条件を加えてもやはり圏となる。
例えば対象を群に限定した圏がある。
...
#figure(
  ...
  #ref(<1ac0d7acb204441885a656784e118d78>)において対象を群としても圏を成す。
] <ff1d9bc304f442f19f37a9138de7c97b>

#proof[
  #ref(<1ac0d7acb204441885a656784e118d78>)と同じ。
]

全ての集合について考えることがあるように全ての群も考えることもある為、#ref(<ff1d9bc304f442f19f37a9138de7c97b>)の定める圏に対して群の圏という名前を与えて自由に使えるようにしておきたいところである。
しかし、冷静に考えてみれば群と群の間に単なる写像を考えることは全くと言って良い程無い。
対象を群と言う構造を持つものに限定したのであれば射も準同型写像と言う構造を持つものに限定すべきであるというのは自然な考えだろう。
そして、これは単に自然というだけでなくより実用性のある構成である。
どのように実用性があるのかについては現時点では触れないが、兎も角射を準同型写像に限定するのであれば#ref(<ff1d9bc304f442f19f37a9138de7c97b>)に比べて示さなければならないことは増える。
何故なら、恒等射や合成射が準同型写像にならない場合は圏として破綻するからだ。
...
#figure(
  ...
  恒等写像は群に対しての準同型写像である。
] <cca7adbbdd3a496f8d5597eb753d5c45>

#proof[
  群$G$に対する恒等写像$op("id"): G -> G$を$op("id")(g) = g$で定義すれば$op("id")(g_1 g_2) = g_1 g_2 = op("id")(g_1) op("id")(g_2)$より準同型写像となる。
]
...
#figure(
  ...
  群に対する準同型写像の合成写像もまた準同型写像である。
] <4b9a3070f34e4510b581b57bbd8106eb>

#proof[
  $G_1, G_2, G_3$を群、$f_1: G_1 -> G_2, f_2: G_2 -> G_3$を準同型写像とする。
  このとき、任意の$g_1, g_2 in G_1$に対して$f_2(f_1(g_1 g_2)) = f_2(f_1(g_1) f_1(g_2)) = f_2(f_1(g_1)) f_2(f_1(g_2))$が成り立つ。
  よって、$(f_2 compose f_1)(g_1 g_2) = (f_2 compose f_1)(g_1) (f_2 compose f_1)(g_2)$より$f_2 compose f_1$は準同型写像となる。
]
...
#figure(
  ...
  対象を群、射を準同型写像、恒等射を恒等写像、合成射を合成写像で定めれば、これは圏を成す。
] <5ca92189ec52453aa161a2103b608bc4>

#proof[
  #ref(<cca7adbbdd3a496f8d5597eb753d5c45>)と#ref(<4b9a3070f34e4510b581b57bbd8106eb>)より恒等写像や合成写像が射であることが分かる。
  単位律と結合律が成り立つことは#ref(<1ac0d7acb204441885a656784e118d78>)と同様に確かめられる。
]

#ref(<5ca92189ec52453aa161a2103b608bc4>)のように、対象が代数的構造ならば射は準同型写像とすることが一般的である。
...
#figure(
  ...
  #ref(<5ca92189ec52453aa161a2103b608bc4>)が定める圏を群の圏$"Grp"$とする。
]
...
全ての対象には恒等射が存在しなくてはならないが、反対に全ての射が恒等射であれば射の合成は自然に定まる。
このような圏を離散圏と言う。
...
離散圏は射の類や射の合成が自明なものしか存在しないので圏として何ら有用な情報を持たず、結局のところ唯の類である。
言い方を変えれば、任意の類を離散圏と見做すことが出来る。

#figure(
  kind: mathblock.kind,
  supplement: mathblock.proposition,
)[
  $X$を類とした時、以下のように定義される$X$は圏である。

  + 対象の類は$X$である。
  + 任意の対象$a, b in X$について、$a = b$の場合は${1_a}$を、$a != b$の場合は${}$を、$a$から$b$への射の類$b^a$として定義する。
  + 任意の対象$a in X$について、$a$の恒等射$1_a in a^a$は唯一存在する射である。
  + 任意の対象$a, b, c in X$と射$f in a^b, g in b^c$について、$f$と$g$の合成射$f compose g in a^c$は唯一存在する射である。
] <732b707387b74d419af0f8cacf7d3c32>

#proof[
  任意の対象$a in X$に対して$a$から$a$への射が一意に存在することは射の定義より自明である。
  同様に、任意の対象$a, b, c in X$と射$f in a^b, g in b^c$について考えれば、$f$と$g$が存在することより$a = b = c$であり、よって$a$から$b$への射は一意に存在するので合成射の定義は成り立つ。
  任意の対象$a, b in X$について$a$から$b$への射は存在するならただ1つであり、よって明らかに単位律と結合律を満たす。
]

以降、集合$X$について離散圏$X$と言った時は#ref(<732b707387b74d419af0f8cacf7d3c32>)によって$X$を圏と見做したものとする。
例えば離散圏$3$は対象が$0, 1, 2 in 3$で射が$1_0, 1_1, 1_2 in op("Mor")(3)$であるような圏のことである。
