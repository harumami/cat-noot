...
#figure(
  ...
  + 対象の類$C$が存在する。
  + 任意の対象$a, b in C$に対して$b$から$a$への射の類$a^b$が存在する。
...
また、略記として、$op("cod")(f) = a, op("dom")(f) = b$であることを$f: a <- b$や$f: b -> a$、$1_a$を単に$1$、$f compose g$の$compose$を除いて$f g$とすることもある。
...
#figure(
  ...
  また、$C$の小さい大きいに関わらず、任意の対象$a, b in C$に対して$b$から$a$への射の類$a^b$が集合であることを局所的に小さいと言う。
]
...
#figure(
  ...
  任意の圏$C$について、頂点の類を$C$、辺の類を$op("Mor")(C)$、始点$op("sou")$を$op("dom")$、終点$op("tar")$を$op("cod")$で定義すると、これは擬箙を成す。
...
#figure(
  ...
  - 対象の類$C$は集合の類である。
  - 任意の対象$a, b in C$に対して$b$から$a$への射の類$a^b$は$b$から$a$への写像の集合で定義される。
