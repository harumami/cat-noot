...
#show figure.where(kind: "mathblock"): it => align(
  left,
...
自然数や実数には大小関係$lt.eq$という二項関係がある。
この関係に基づいて自然数の集合$NN$や実数の集合$RR$を直線上の点として表したものは数直線と呼ばれる。
このような図形は抽象概念の理解の良い助けとなるだろう。
...
例えば、$RR$上の全順序は一般に$lt.eq$なので通常は$lt.eq$は省略して単に前順序集合$RR$と言う。
前順序集合の他の例として$NN$があるが、以降では$NN$とその上での前順序を実際に構成してみよう。

#definition(caption: [ペアノ構造])[
...
#proof[
  ...
  $ P(M) = z in M and forall n in M, s(n) in M $

  とし、任意の集合$M$に対して集合$N(M)$を

  $ N(M) = inter.big { M' subset.eq M | P(M') } $
...
#definition(caption: [自然数])[
  #ref(<f3f1aa06ad0b460f9a900a1e99bf9721>)によって定義されるペアノ構造を$(NN, 0, "suc")$する。
  また、$NN$を自然数の集合、$NN$の元を自然数と言う。
] <8eaa0cccd787486294d68396cff2b2a4>

$"suc"(0), "suc"("suc"(0)), "suc"("suc"("suc"(0)))$のように、$0$に$"suc"$を繰り返し適用することで好きなだけ自然数を得られる。
このようにして得られる自然数を順に$1, 2, 3, ... in NN$とする。
これらは具体的には次のような集合である。

$ 0 = emptyset = {} $
$ 1 = "suc"(0) = {} union {0} = {0} $
$ 2 = "suc"(1) = {0} union {1} = {0, 1} $
$ 3 = "suc"(2) = {0, 1} union {2} = {0, 1, 2} $

ここから推察されることは、自然数$n$とは$n$より小さい自然数の集合であるということである。
これを示すための準備としてまず#ref(<f643367ee6ee479bb559182eb9d860e5>)を示す。

#theorem[
  任意の自然数$n_1, n_2 in NN$について、$n_1 subset "suc"(n_2)$ならば$n_1 subset.eq n_2$が成り立つ。
] <f643367ee6ee479bb559182eb9d860e5>

#proof[
  集合$N$を

  $ N = { n in NN | n subset "suc"(n_2) => n subset.eq n_2 } $

  で定義する。
  このとき、$0 = emptyset subset.eq n_2$より$0 in N$となる。
  次に、任意の$n in N$に対して$"suc"(n) = n union {n} in N$であることを示す。
  $"suc"(n) subset "suc"(n_2)$を仮定すれば$n subset "suc"(n_2)$と$n in "suc"(n_2)$の両方が成り立ち、$n in N$を思い出せば$n subset "suc"(n_2)$から$n subset.eq n_2$が成り立つ。
  さらに、$n in "suc"(n_2)$からは$n in n_2$と$n = n_2$のどちらかが成り立ち、$n in n_2$ならば$n subset.eq n_2$と合わせて考えて$"suc"(n) subset.eq n_2$が成り立つ。
  残った$n = n_2$の場合では仮定の$"suc"(n) subset "suc"(n_2)$と矛盾し、よって$"suc"(n) in N$が成り立つ。
  以上の議論と#ref(<b89addb4bee04500a3caaa5d8ab4267c>)より$N = NN$となるので、$n_1 in NN = N$から$n_1 subset "suc"(n_2) => n_1 subset.eq n_2$が示された。
]
...
#proof[
  ...
  よって、$n_2$で成り立つという仮定の下で$"suc"(n_2)$でも成り立つことを示せば良い。
  $n_1 in "suc"(n_2)$ならば$n_1 in n_2$と$n_1 = n_2$の何れかが成り立ち、どちらであっても$n_1 subset "suc"(n_2)$である。
  一方、$n_1 subset "suc"(n_2)$ならば#ref(<f643367ee6ee479bb559182eb9d860e5>)より$n_1 subset.eq n_2$であり、$n_1 subset n_2$と$n_1 = n_2$の両方で$n_1 in "suc"(n_2)$が成り立つ。
]
...
#theorem[
  ...
  $ n_1 R n_2 <=> n_1 = n_2 or n_1 in n_2 $
...
任意の自然数$n in NN$について$n in "suc"(n)$は自明なので、#ref(<4662a662e47e4a6e8f35cd3889b99453>)は自然な大小関係$lt.eq$と同値であることが分かる。
より正確に言えば、#ref(<8eaa0cccd787486294d68396cff2b2a4>)における$NN$の自然な前順序とは#ref(<4662a662e47e4a6e8f35cd3889b99453>)が定める関係のことである。

#definition(caption: [自然数の前順序])[
...
そのようなインターフェース、代数的構造の1つが、#ref(<ba9d86c3c1d845e18ab46f2f3617bfbc>)で議論する群である。
...
一般に全単射には逆写像が存在するので、それを$f^(- 1)$とすれば

$ g_1 g_2 = f^(- 1)(f(g_1) f(g_2)) $
...
群によって二項演算を代数的構造として抽象化できることは既に見たが、果たしてこれが正しい抽象化なのかは疑問の余地が残る。
...
#theorem[
  $A in A$を満たす集合$A$はZF公理系に矛盾する。
] <632e5265d7f9495ca6ee7d0ba931d17a>

#proof[
  正則性公理より

  $ forall X != emptyset, exists x in X, X inter x = emptyset $

  $X = {A}$とすれば、${A} != emptyset$より

  $ exists x in {A}, {A} inter x = emptyset $

  $x in {A}$を満たすのは$x = A$のみなので

  $ {A} inter A = emptyset $

  よって、$A in emptyset$となり矛盾。
]
