...
擬箙は箙における頂点の集合や辺の集合を類に変えたものである。
全ての集合の「集まり」は類であるので、これによって集合の類と写像の類による擬箙を考えることが出来る。

#figure(
  kind: mathblock.kind,
  supplement: mathblock.proposition,
)[
  頂点の類$Q_0$を集合の類、辺の類$Q_1$を写像の類、辺$e in Q_1$の始点$op("sou")(e) in Q_0$を$e$の始域、終点$op("tar")(e) in Q_0$を$e$の終域とすれば、これは擬箙となる。
...
== 代数的構造
#ref(<2da7dc6cfbbd4065876df642d8c8d4c0>)では前順序集合、#ref(<ba9d86c3c1d845e18ab46f2f3617bfbc>)では群、#ref(<e36014ced53a4b16a33373f8acea2911>)では箙という、定義によって定まる構造を持った集合について議論してきた。
このような、定義によって定まる抽象的な構造のことを一般に代数的構造と言う。
代数的構造を考える意義は、議論における共通言語を与え、証明を再利用し、異なる概念の比較を可能にすることにある。
しかし、ここで、ある単純な問が生まれる。
それは、代数的構造全体も、また何らかの構造を有しているのではないかという直感である。
もし複数の分野に共通する代数的構造を定義できたなら、それは非常に有用だろう。
具体的構成によって理論の無矛盾性を保証する役割が集合論にあるように、抽象代数学の共通のインターフェースとしての役割を見出せる。
そして、それこそが圏論なのである。

== 圏 <08b37d846986456dbdcbe66408a829ee>

様々な構造を一般化する為の代数的構造として、圏では対象とその間の射というものを考える。
圏の定義を簡単に言うならば、モノイドのように演算が定まった箙である。
...
#figure(
  ...
  + 対象の類$C$が存在する。
  + 任意の対象$a, b in C$に対して、$a$の$b$による射の類$a^b$が存在する。
  + 任意の対象$a in C$に対して、恒等射$1_a in a^a$が存在する。
  + 任意の対象$a, b, c in C$と射$f in a^b, g in b^c$に対して、$f$と$g$の合成射$f compose g in a^c$が存在する。
  + 任意の対象$a, b in C$と射$f in a^b$に対して、単位律$1_a compose f = f = f compose 1_b$が成り立つ。
  + 任意の対象$a, b, c, d in C$と射$f in a^b, g in b^c, h in c^d$に対して、結合律$(f compose g) compose h = f compose (g compose h)$が成り立つ。
] <c1795453f0e04184b9fdbf6fdfd53cc8>

圏の定義には様々な流派があり、用いる記号の違いが少なくない。
よって、このノートでは基本的に独自の記法にのみ言及する。
一般に$a^b$は一般には$C(b, a)$と書かれ、特に$f in C(b, a)$は$b$から$a$の射と言われる。
$b$から$a$の射の類を$a^b$と書き、それを$a$の$b$による射と呼ぶのはこのノート独自である。
...
#figure(
  ...
  圏$C$の射$f in op("Mor")(C)$に対して$f in a^b$を満たすような$a, b in C$をそれぞれ領域、余領域と言い、$op("tar")(f) = a, op("cotar")(f) = b$で表す。
] <fd6c44cc7606498aa3297da25435bb6e>

なお、一般には領域を余域、余領域を域と言い、それぞれを$op("cod"), op("dom")$で表す。
領域、余領域、$op("tar"), op("cotar")$は全てこのノート独自の記法である。
#ref(<fd6c44cc7606498aa3297da25435bb6e>)が成立するためには領域と余領域が一意に定まらなくてはならないが、これは#ref(<e36014ced53a4b16a33373f8acea2911>)と同様の理由で問題にならない。
また、略記として、$op("tar")(f) = a, op("cotar")(f) = b$であることを$f: a <- b$、$1_a$を単に$1$、$f compose g$の$compose$を除いて$f g$とすることもある。
...
#figure(
  ...
  また、$C$の小さい大きいに関わらず、任意の対象$a, b in C$に対して$a$の$b$による射の類$a^b$が集合であることを局所的に小さいと言う。
]

例えば、#ref(<6a578525efa943219d6c53d204d4e2d2>)で見るような集合の圏$"Set"$は大きく、同時に局所的に小さい。
類を誤って集合として扱うことは矛盾の原因となってしまうため、本来は慎重に議論すべきである。
...
#ref(<c1795453f0e04184b9fdbf6fdfd53cc8>)から箙の名残を感じられるかもしれない。
実は、任意の圏から擬箙を構成することができる。

#figure(
  kind: mathblock.kind,
  supplement: mathblock.proposition,
)[
  任意の圏$C$について、頂点の類を$C$、辺の類を$op("Mor")(C)$、始点$"sou"$を$"cotar"$、終点$"tar"$を$"tar"$で定義すると、これは擬箙を成す。
...
また、当たり前のことであるが、全ての対象や射を図示せずとも注目している有限個の対象と射のみで有向グラフを作ることも出来る。
この方法によって圏を有向グラフとして図示したものは可換図式と呼ばれる。
例えば#ref(<b780981f41d5475eaf0410a51b5189f5>)も可換図式として見ることが出来る。
なお、恒等射は合成射は一般に省略される。

= 圏の具体例 <94c440b9d4ba44348cc8d3d89d0d3dee>

#ref(<94c440b9d4ba44348cc8d3d89d0d3dee>)では具体例を通じて様々な概念が圏を成すことを確認する。

== 集合の圏 <6a578525efa943219d6c53d204d4e2d2>

対象を集合、射を写像とする圏を考えることができる。
...
#figure(
  ...
  - 対象の類$C$は集合の類である。
  - 任意の対象$a, b in C$に対して$a$の$b$による射の類$a^b$は$a$から$b$への写像の集合で定義される。
