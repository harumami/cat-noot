...
また、箙を擬箙に拡張することでわざわざ全ての集合の分だけ頂点があるグラフを考えられるようにしたが、そもそも頂点が無数にあるグラフを描くことは出来ない。
つまり、グラフとして図示するのは常にある箙或いは擬箙の部分箙であるということだ。
例えば#ref(<18b214ca66bb4ea88a339b4f66094b25>)を#ref(<93dcf43de2c348ca8cc30a65d43d40e4>)によって定義される擬箙の部分箙であるとした時、これは$X, Y, Z$が集合、$f: X -> Y, g: Y -> Z$がその間の写像を意味する変数として定義していると解釈する。
...
圏とはモノイドに似た演算を備えた箙であると言ったが、実は任意の圏から擬箙を構成することができる。
...
また、特に可換図式と言った時はその図式内の領域と余領域を共有する射が等しいことを意味する。
